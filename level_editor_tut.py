...
class FileFormat(str, Enum):
    ...
    def next(self):
        format_list = list(self.__class__)
        index = format_list.index(self)
        next_index = (index + 1) % len(format_list)
        return format_list[next_index]
...
for i, img in enumerate(img_list):
    ...
    tile_button = button.Button(
        SCREEN_WIDTH + (75 * button_col) + 50, 75 * button_row + 50, img, 1
    )
    button_list.append(tile_button)

current_file_format = FileFormat.CSV
...
while run:
    ...
    draw_text(
        f"Level: {level}    Format: {current_file_format}",
        font,
        WHITE,
        10,
    ...
    if save_button.draw(screen):
        # save level data
        file = Path(f"level{level}_data.{current_file_format}")
        if current_file_format == FileFormat.JSON:
            file.write_text(json.dumps(world_data))
    ...
    if load_button.draw(screen):
        ...
        file = Path(f"level{level}_data.{current_file_format}")
        if current_file_format == FileFormat.JSON:
            world_data = json.loads(file.read_text())
        elif current_file_format == FileFormat.CSV:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_f:
                current_file_format = current_file_format.next()
